...
export default function OrdersManager() {
  ...
  // 🗓️ Thêm state quản lý tháng/năm
  ...
  // 🧾 Xuất Excel chia sheet theo tháng/năm
  async function exportToExcel() {
    const workbook = new ExcelJS.Workbook();
    const grouped = {};

    orders.forEach((o) => {
      const d = new Date(o.receivedDate || o.createdAt || Date.now());
    ...
    for (const [key, list] of Object.entries(grouped)) {
      const sheet = workbook.addWorksheet(`Tháng ${key}`);
      const headers = [
        "STT",
        "Trạng thái",
        "Ngày nhận",
    ...
    workbook.getWorksheet(1).addImage(logoId, {
      tl: { col: 8, row: 2 },
    ...
    saveAs(
      ...
      `DonHang_${new Date().getFullYear()}.xlsx`
    );
  }
                        ...
                        <td className="px-4 py-2 align-top">
                          <ul className="list-disc ml-4">
                            {(o.items || []).map((it, i) => {
                              ...
                              return <li key={i}>{prod ? prod.name : "Sản phẩm đã xóa"}</li>;
                            })}
                          </ul>
                        </td>
                        <td className="px-4 py-2 align-top">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 pt-30">
          ...
          >
            ...
            <h3 className="text-xl font-semibold mb-4">
              {editing.id ? "Chỉnh sửa đơn hàng" : "Tạo đơn hàng"}
            </h3>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div>
                <label className="block font-medium">Sản phẩm</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-medium">Ngày nhận</label>
                  <input
                    type="date"
                    value={editing.receivedDate || ""}
                    onChange={(e) =>
                      setEditing((p) => ({ ...p, receivedDate: e.target.value }))
                    }
                    className="border rounded px-2 py-1 w-full"
                  />
                </div>
                <div>
                  <label className="block font-medium">Ngày giao</label>
                  <input
                    type="date"
                    value={editing.deliverDate || ""}
                    onChange={(e) =>
                      setEditing((p) => ({ ...p, deliverDate: e.target.value }))
                    }
                    className="border rounded px-2 py-1 w-full"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-medium">Thanh toán</label>
                  <select
                    value={editing.paymentMethod}
                    onChange={(e) =>
                      setEditing((p) => ({ ...p, paymentMethod: e.target.value }))
                    }
                    className="border rounded px-2 py-1 w-full"
                  >
