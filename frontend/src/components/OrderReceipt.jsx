...
import { PlusCircle, Trash2, ImageDown, ChevronDown, Save } from "lucide-react"; // Import Save Icon
...
const OrderReceipt = () => {
  ...
  // Tính tiền từng món (đã trừ sale từng món)
  ...
  // --- 🔥 HÀM MỚI: LƯU ĐƠN HÀNG VÀO DATABASE ---
  const handleSaveOrder = async () => {
    if (!form.customerId) return toast.error("Vui lòng chọn khách hàng trước khi lưu");
    if (form.items.length === 0) return toast.error("Đơn hàng chưa có sản phẩm");

    setIsSaving(true);
    try {
        // Chuẩn bị dữ liệu để gửi lên backend
        // Backend cần: items[{productId, size, quantity, price}], status, receivedDate, deliverDate, total
        ...
        const payload = {
            ...
            paymentMethod: "COD", // Hoặc thêm field chọn paymentMethod vào form Receipt
            total: totalWithShip, // Gửi tổng tiền cuối cùng (đã tính ship, sale)
            items: form.items.map(item => {
                const product = products.find(p => String(p._id || p.id) === String(item.productId));
                const originalPrice = product ? product.price : 0;
                
                // Tính đơn giá sau khi giảm (để backend lưu đúng giá trị thực thu)
                const discountedUnitPrice = originalPrice * (1 - (item.sale || 0) / 100);

                return {
                    productId: item.productId,
                    size: item.size || "",
                    quantity: item.quantity,
                    price: discountedUnitPrice // Gửi giá đã giảm
                };
            })
        };
  ...
  const ReceiptContent = () => (
    <div className="w-[1000px] bg-white text-gray-900 font-sans p-10 mx-auto">
        ...
        <div className="flex justify-end mt-10 pt-6 border-t-2 border-gray-300">
           <div className="w-[450px] space-y-3 text-right">
        ...
        {/* FOOTER */}
        <div className="mt-12 text-center text-gray-400 italic text-base">
    ...
    >
      <div className="flex justify-end mb-4 gap-3">
        ...
        <button
          ...
          disabled={isSaving}
          className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition shadow-sm font-bold disabled:opacity-50"
        >
          <Save className="w-5 h-5" />
          <span className="hidden sm:inline">{isSaving ? "Đang lưu..." : "Lưu đơn hàng"}</span>
        </button>
        ...
        >
          ...
          <span className="hidden sm:inline">Xuất Ảnh Phiếu</span>
        </button>
      </div>
