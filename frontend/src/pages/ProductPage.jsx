...
const ProductPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white text-gray-900 pt-30">
          ...
          {allThumbnails.length > 1 && (
            <div className="relative flex items-center justify-center mt-4 w-full max-w-[464px]">
              {canScroll && startIndex > 0 && (
                <button type="button" onClick={handlePrev} className="absolute -left-5 z-20 bg-white/90 hover:bg-white text-gray-800 shadow-md rounded-full p-2">
                ...
                {visibleThumbnails.map((thumb, index) => (
                  <motion.img 
                    ...
                    className={`w-[116px] h-[140px] object-cover rounded-md cursor-pointer border-2 transition-all duration-300 ${
                        mainImage === thumb ? "border-gray-900 scale-110 shadow-md z-10" : "border-gray-200 hover:border-gray-500"
                    }`} 
                  />
                ))}
              </div>
              ...
              {canScroll && startIndex + visibleCount < allThumbnails.length && (
                <button type="button" onClick={handleNext} className="absolute -right-5 z-20 bg-white/90 hover:bg-white text-gray-800 shadow-md rounded-full p-2">
        ...
        <div className="flex-1 flex flex-col">
          <div className="p-6 rounded-xl border border-gray-200 bg-white shadow-lg space-y-4">
            ...
            <div className="mt-4 flex gap-3">
              ...
              >
                <ShoppingCart size={20} className="mr-1" /> Thêm vào giỏ
              </button>
            </div>
          </div>
      ...
      <div className="mt-16">
        <PeopleAlsoBought key={id} excludeIds={memoizedExcludeIds} />
      </div>
        ...
        {zoomImage && (
          ...
          >
            ...
            >
              ...
              >
                {/* 👇 SỬA Ở ĐÂY: Dùng centerView để căn giữa khi zoom bằng slider */}
                {({ zoomIn, zoomOut, resetTransform, centerView, state }) => (
                  <>
                    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-white px-6 py-3 rounded-full flex items-center gap-4 z-[110] border border-white/10">
                      <button 
                        onClick={() => { resetTransform(); setSliderValue(1); }} 
                      ...
                      <input 
                        ...
                        onChange={(e) => { 
                            ...
                            setSliderValue(val); 
                            // ✅ Dùng centerView(val, 0) để zoom từ TÂM và không có animation (0ms) để mượt
                            centerView(val, 0); 
                        }} 
                    ...
                    {/* 👇 SỬA Ở ĐÂY: Bỏ contentClass để tránh xung đột CSS căn giữa của thư viện */}
