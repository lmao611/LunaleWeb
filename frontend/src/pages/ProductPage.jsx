...
const ProductPage = () => {
  ...
  const { cart } = useCartStore(); // Chỉ lấy cart để check excludeIds
  ...
  // Memoize danh sách ID cần loại trừ ở phần "Có thể bạn cũng thích"
  ...
  const originalPriceDisplay =
    originalPrice.toLocaleString("vi-VN", { maximumFractionDigits: 0 }) + "đ";
  const discountedPriceDisplay =
    discountedPrice.toLocaleString("vi-VN", { maximumFractionDigits: 0 }) + "đ";

  // --- Logic Ảnh & Slider Thumbnail ---
  const allThumbnails = [
    product.image,
  ...
  // ✅ HANDLER: MỞ POPUP GIỎ HÀNG (Thay vì thêm trực tiếp)
  ...
  // Ngăn chặn sự kiện lan truyền khi click vào các element con
        ...
        <div className="flex-1 flex flex-col">
          <div className="p-6 rounded-xl border border-gray-200 bg-white shadow-lg space-y-4">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {isSale ? (
                <>
                  ...
                  <p className="text-gray-400 text-xl line-through decoration-2 decoration-gray-300">
            ...
            <div className="mt-4 flex gap-3">
              ...
              {/* Nút thêm giỏ hàng gọi hàm mở Popup */}
              <button
                type="button"
              ...
              >
                {({ zoomIn, zoomOut, resetTransform, centerView, state }) => (
                  <>
                    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-white px-6 py-3 rounded-full flex items-center gap-4 z-[110] border border-white/10">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setSliderValue(val);
                          // ✅ Dùng centerView để zoom từ tâm, tránh lệch ảnh
                          centerView(val, 0);
                        }}
                    ...
                    {/* ✅ Component chứa ảnh Zoom (Bỏ class flex để tránh xung đột) */}
                    <TransformComponent
