...
const ProductionDetailPage = () => {
  ...
  const renderSizeInputs = (row, index, category, isReadOnly) => {
    ...
    return (
      <>
        <td className={`border border-black p-0 min-w-[30px] md:min-w-[40px] bg-gray-100 font-bold ${isTonCuoiKy ? 'text-red-600' : ''}`}>
          {total === 0 ? "-" : total}
        </td>
        {['S', 'M', 'L', 'XL'].map(size => {
          const val = row[category]?.[size];
          return (
            <td key={size} className="border border-black p-0 min-w-[30px] md:min-w-[40px]">
              <input
                type="number"
                value={val || ""}
                placeholder="-"
                ...
                disabled={isReadOnly}
                className="w-full h-full text-center bg-transparent outline-none py-1.5 md:py-2 disabled:bg-transparent text-[10px] md:text-sm"
              />
            </td>
          );
        })}
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-gray-900 p-3 md:p-4 pt-20 md:pt-24 text-xs md:text-sm pb-20">
      <div className="max-w-[1800px] mx-auto">
        <div className="flex justify-between items-center mb-4 md:mb-6">
          ...
          <div className="flex items-center gap-2 text-xs md:text-sm font-medium">
            {saveStatus === "saving" ? (
              <span className="flex items-center text-blue-600 bg-blue-50 px-3 md:px-4 py-1.5 md:py-2 rounded-md">
                <Loader2 className="mr-1.5 md:mr-2 animate-spin" size={16} /> Đang lưu...
              </span>
            ) : (
              <span className="flex items-center text-emerald-600 bg-emerald-50 px-3 md:px-4 py-1.5 md:py-2 rounded-md">
                <Check className="mr-1.5 md:mr-2" size={16} /> Đã lưu tự động
              </span>
            )}
          </div>
        </div>

        <h1 className="text-xl md:text-3xl font-bold mb-6 md:mb-8 text-center uppercase tracking-widest text-black">
          CHI TIẾT SẢN XUẤT - {product.name}
        </h1>

        <div className="flex flex-col 2xl:flex-row gap-6 md:gap-8 items-start">
          <div className="w-full 2xl:w-1/2 flex flex-col gap-4 md:gap-6 min-w-0">
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
              <div className="w-full sm:w-40 md:w-48 border-2 border-black bg-white flex flex-col items-center justify-center p-2 shadow-sm shrink-0">
                <span className="font-bold text-center uppercase mb-2 text-sm md:text-base">{product.name}</span>
                <img 
              ...
              <div className="flex-1 overflow-x-auto shadow-sm min-w-0">
                <table className="w-full border-collapse text-center border-2 border-black bg-white text-[10px] md:text-xs whitespace-nowrap">
                  <thead>
                    <tr>
                      <th className="border border-black p-1 md:p-2 bg-[#8FAADC]" colSpan="5">TỒN ĐẦU KỲ</th>
                      <th className="border border-black p-1 md:p-2 bg-[#A9D08E]" rowSpan="2">NGÀY NHẬP</th>
                      <th className="border border-black p-1 md:p-2 bg-[#A9D08E]" colSpan="5">NHẬP TRONG KỲ</th>
                      <th className="border border-black p-1 md:p-2 bg-[#F4B084]" rowSpan="2">NGÀY XUẤT</th>
                      <th className="border border-black p-1 md:p-2 bg-[#F4B084]" colSpan="5">XUẤT TRONG KỲ</th>
                      <th className="border border-black p-1 md:p-2 bg-[#8FAADC]" colSpan="5">TỒN CUỐI KỲ</th>
                      <th className="border border-black p-1 md:p-2 bg-red-100" rowSpan="2">Xóa</th>
                    </tr>
                    <tr>
                      {['Tổng', 'S', 'M', 'L', 'XL'].map((h, i) => <th key={`td-${i}`} className="border border-black p-0.5 md:p-1 bg-gray-100">{h}</th>)}
                      {['Tổng', 'S', 'M', 'L', 'XL'].map((h, i) => <th key={`n-${i}`} className="border border-black p-0.5 md:p-1 bg-gray-100">{h}</th>)}
                      {['Tổng', 'S', 'M', 'L', 'XL'].map((h, i) => <th key={`x-${i}`} className="border border-black p-0.5 md:p-1 bg-gray-100">{h}</th>)}
                      {['Tổng', 'S', 'M', 'L', 'XL'].map((h, i) => <th key={`tc-${i}`} className="border border-black p-0.5 md:p-1 bg-gray-100">{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {inventory.map((row, index) => (
                      <tr key={index} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="border border-black p-0 min-w-[80px] md:min-w-[110px]">
                          <MaskedDateInput 
                            value={row.ngayNhap || "-"} 
                            onChange={(val) => handleInventoryChange(index, "ngayNhap", null, val)}
                            className="w-full h-full text-center bg-transparent outline-none py-1.5 md:py-2 text-[10px] md:text-xs"
                          />
                        </td>
                        ...
                        <td className="border border-black p-0 align-middle">
                          {index > 0 && (
                            <button
                              ...
                              className="text-red-500 hover:text-red-700 w-full h-full flex items-center justify-center py-1.5 md:py-2"
                              title="Xóa hàng này"
                            >
                              <Trash2 size={14} className="max-md:w-3 max-md:h-3" />
                            </button>
                          )}
                        </td>
                      </tr>
                ...
                <div className="bg-white p-2 border-t-0 border-2 border-black">
                  <button 
                    ...
                    className="flex items-center text-blue-600 hover:text-blue-800 text-xs md:text-sm font-semibold transition-colors"
                  >
                    <Plus size={16} className="mr-1 max-md:w-4 max-md:h-4" /> Thêm dòng xuất/nhập
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="w-full 2xl:w-1/2 flex flex-col gap-4 md:gap-6 min-w-0">
              ...
              return (
                <div key={batchIndex} className="overflow-x-auto shadow-sm min-w-0 relative">
                  <div className="flex justify-between items-end mb-1">
                    <span className="text-[10px] md:text-xs font-semibold text-gray-500">Bảng đợt {batchIndex + 1}</span>
                    <button 
                      onClick={() => removeBatch(batchIndex)} 
                      className="text-red-500 hover:text-red-700 text-[10px] md:text-xs font-semibold flex items-center transition-colors"
                    >
                      <Trash2 size={12} className="mr-1 max-md:w-3 max-md:h-3"/> Xóa đợt này
                    </button>
                  </div>
                  
                  <table className="w-full border-collapse text-center border-2 border-black bg-white whitespace-nowrap text-[10px] md:text-xs">
                    <thead>
                      <tr>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] text-red-600 uppercase min-w-[120px] md:w-48 font-bold">
                          {product.name} ĐỢT {batchIndex + 1}
                        </th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] min-w-[80px] md:w-32 font-bold">NGUỒN NHẬP</th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] min-w-[80px] md:w-32 font-bold">NGÀY NHẬP</th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] min-w-[90px] md:w-32 font-bold">SỐ LƯỢNG (MÉT)</th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] min-w-[80px] md:w-32 font-bold">GIÁ</th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] min-w-[100px] md:w-40 font-bold">TỔNG</th>
                        <th className="border border-black p-1.5 md:p-3 bg-[#FFC000] w-8 md:w-10 font-bold">Xóa</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={itemIndex} className="hover:bg-gray-50 transition-colors">
                          <td className="border border-black p-0">
                            <input 
                              ...
                              className="w-full h-full text-center bg-transparent outline-none py-1.5 md:py-2 font-semibold"
                            />
                          </td>
                          <td className="border border-black p-0">
                            <input 
                              ...
                              className="w-full h-full text-center bg-transparent outline-none py-1.5 md:py-2"
                            />
                          </td>
                          <td className="border border-black p-0">
                            <MaskedDateInput 
                          ...
                          <td className="border border-black p-1.5 md:p-2 font-medium">
                      ...
                      <tr className="bg-gray-50">
                        <td colSpan="7" className="border border-black p-1.5 md:p-2">
                          <button 
                            onClick={() => addBatchItem(batchIndex)}
                            className="flex items-center text-blue-600 hover:text-blue-800 text-xs md:text-sm font-semibold transition-colors mx-auto"
                          >
                            <Plus size={16} className="mr-1 max-md:w-4 max-md:h-4" /> Thêm nguyên liệu
                          </button>
                        </td>
                      </tr>
                      <tr className="bg-[#FFC000] font-bold">
                        <td colSpan="5" className="border border-black p-2 md:p-3 text-right uppercase pr-3 md:pr-6 text-[10px] md:text-xs">Tổng Cộng</td>
                        <td colSpan="2" className="border border-black p-2 md:p-3 text-red-600 text-sm md:text-lg text-left">{formatCurrency(batchTotal)} ₫</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-stretch gap-3 md:gap-4 mt-2">
              <button 
                onClick={addBatch}
                className="bg-[#A9D08E] border-2 border-black text-black p-2 md:p-3 font-bold hover:bg-[#96c179] transition-all rounded-sm flex-1 shadow-sm text-xs md:text-base"
              >
                THÊM ĐỢT MỚI ( + )
              </button>
              <div className="flex-1 bg-[#FFC000] border-2 border-black text-black p-2 md:p-3 font-bold flex items-center justify-between shadow-sm text-sm md:text-lg px-3 md:px-6 uppercase">
                <span className="text-xs md:text-sm">Tổng tất cả các đợt:</span>
